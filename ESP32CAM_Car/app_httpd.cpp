...
//        "}"
//        "button:active {"
//          "transform: translateY(2px);"    // 按下時往下位移 2px
//          "opacity: 0.8;"                  // 稍微變淡，強調按下效果
//          "box-shadow: inset 0 4px 6px rgba(0,0,0,0.2);"  // 內凹投影
//        "}"
//      "</style>";
...
//順逆時針
 ...
 page += "<button style=background-color:Tan;width:135px;height:80px onmousedown=getsend('counterclockwise') onmouseup=getsend('stop') ontouchstart=getsend('counterclockwise') ontouchend=getsend('stop') ><b>Turn counterclockwise</b></button> </p>"; 

 //手臂
 page += "<p align=center>";
 page += "Rocker arm<br>";
 page += "<button style=background-color:PowderBlue;width:90px;height:80px ;box-shadow:10px 10px; onmousedown=getsend('a_forward') onmouseup=getsend('stop') ontouchstart=getsend('a_forward') ontouchend=getsend('stop') ><b>Forward</b></button>&nbsp;";
 page += "<button style=background-color:PowderBlue;width:90px;height:80px onmousedown=getsend('a_reverse') onmouseup=getsend('stop') ontouchstart=getsend('a_reverse') ontouchend=getsend('stop') ><b>Reverse1/2</b></button> </p>"; 
...
static esp_err_t go_handler(httpd_req_t *req){ //往前
    WheelAct(HIGH, LOW, HIGH, LOW);
    Serial.println("Go");
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, "OK", 2);
}
static esp_err_t back_handler(httpd_req_t *req){//後退
...
static esp_err_t left_handler(httpd_req_t *req){//左轉
    WheelAct(HIGH, LOW, LOW, HIGH);
    Serial.println("Left");
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, "OK", 2);
}
static esp_err_t right_handler(httpd_req_t *req){//右轉
...
static esp_err_t stop_handler(httpd_req_t *req){ //停止
...
static esp_err_t ledon_handler(httpd_req_t *req){ //LED燈開
    digitalWrite(gpLed, HIGH);
    Serial.println("LED ON");
    httpd_resp_set_type(req, "text/html");
    return httpd_resp_send(req, "OK", 2);
}
static esp_err_t ledoff_handler(httpd_req_t *req){ //LED燈關
...
static esp_err_t a_forward_handler(httpd_req_t *req){ //A馬達正轉
